...
ZIP: {
  ...
  numberOrEn: {
	  validator: function (value, param) {
	     	return /(^([1-9]|[a-zA-Z]){1,6}$)/.test(value);
	     },
	     message:'密码由不超过六位的数字或字母组成'
  },
  password: {
	  validator: function (value, param) {
		  return /^([0-9]|[A-Z]|[a-z]){5,12}$/.test(value);
	     },
	     message:'密码应为5到12位的数字或英文组合'
  },
  money:{
    validator: function (value, param) {
  ...
  route:{
	 validator: function (value, param) {
	     return /^\/[a-zA-Z]{0,30}([\/a-zA-Z\w]){1,30}$/.test(value);
	   },
	  message:'请输入正确的路径'
	  },
  integer:{
